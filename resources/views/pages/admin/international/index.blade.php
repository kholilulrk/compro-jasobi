        ...
        <div class="animated fadeIn">
            ...
            <div class="card">
                <div class="card-header">
                    <i class="fa fa-edit"></i> international
                                            ...
                                            <td>
                                                <div class="thumbnail">
                                                    <img class="img-thumbnail"
                                                        src="{{ asset($international->showImage()) }}" alt="">
                                                </div>
                                                <span class="text-muted">Publish :
                                                    {{ $international->created_at }}</span>
                                            </td>
                                            <td> {{ $international->url }} </td>
                                            <td> {{ $international->url_website }} </td>
                                            <td class="sorting_1"> <b>{{ $international->title }}</b> </td>
                                            <td>{{ $international->short_description }}</td>
                                            <td>{!! $international->description !!}</td>
                                            <td>
                                                <form
                                                    action=" {{ route('admin.international.update', $international->id) }} "
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <a href=" {{ route('admin.international.show', $international->id) }} "
                                                        class="btn btn-success">
                                                        <i class="fa fa-eye"></i>
                                                    </a>
                                                    <a href=" {{ route('admin.international.edit', $international->id) }} "
                                                        class="btn btn-warning">
                                                        <i class="fa fa-edit"></i>
                                                    </a>
                                                    <form
                                                        action="{{ route('admin.international.destroy', $international->id) }}"
                                                        method="post">
                                                        {{ csrf_field() }}
                                                        {{ method_field('delete') }}
...
<div class="modal fade" id="Modal-tambah" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action=" {{ route('admin.international.store') }} " method="POST" enctype="multipart/form-data">
